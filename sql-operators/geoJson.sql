-- Parse a GeoJSON string as a table using the GeomFromGeoJSON operator
sel GeomFromGeoJSON('{''type'': ''Polygon'', ''coordinates'': [[[-69.99693762899992, 12.577582098000036], [-69.93639075399994, 12.53172435100005], [-69.92467200399994, 12.519232489000046]]]}', 4326)

-- Extract features from GeoJSON document using the JSON_Table operator
sel *
FROM JSON_Table 
(ON (
	sel 
	 1 id
	,cast(geo_lob as JSON) jsonCol
	from
	(
		sel '{
...
USING rowexpr('$.features[*]')
               colexpr('[ {"jsonpath" : "$.geometry",
                           "type" : "VARCHAR(32000)"},
                          {"jsonpath" : "$.properties.ADMIN",
                           "type" : "VARCHAR(50)"},
                          {"jsonpath" : "$.properties.ISO_A3",
                           "type" : "VARCHAR(50)"}]')
) AS JT(id, geom, country_name, code_isoa3);